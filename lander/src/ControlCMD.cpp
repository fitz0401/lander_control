...
namespace ControlCMD
{
    ...
    auto MoveMotor::prepareNrt()->void
        {
    ...
    auto MoveMotor::collectNrt()->void {}
    MoveMotor::~MoveMotor() = default;
    MoveMotor::MoveMotor(const std::string &name)
        {
            //构造函数参数说明，构造函数通过xml的格式定义本条指令的接口，name表示参数名，default表示输入参数，abbreviation表示参数名的缩写(缩写只能单个字符)
            //1 GroupParam下面的各个节点都是输入参数，如果没有给定会使用默认值
            //2 UniqueParam下面的各个节点互斥，有且只能使用其中的一个
    ...
    //Move Leg: along the points obtained by simulation
    struct MoveLegParam
    {
        std::vector<bool> active_motor;			//目标电机
        std::vector<double> begin_pjs;			//起始位置
    ...
    {
        auto &param = std::any_cast<MoveLegParam&>(this->param());
...
//        }
	...
	ARIS_REGISTRATION
	{
        ...
        aris::core::class_<MoveMotor>("MoveMotor")
            .inherit<Plan>()
            ;
